...
#     def is_computation_node(node):
#         return not (
#             NodeClassifier.is_communication(node)
...
class Graph:
    ...
    def show(self, sorted=False):
        ...
        for node in nodes:
            ...
            print(f" {node_type=}, {node_val=}, {node_sort_idx=}")
...
class ComputationCommunicationOverlapSubGraph:
    ...
    def _show(self):
        if self.comp_nodes is not None:
            comp_nodes_val = [node.val for node in self.comp_nodes]
            comp_nodes_idx = [node.__sort_idx__ for node in self.comp_nodes]
            print(f"    comp nodes: val={comp_nodes_val}, idx={comp_nodes_idx}")
        
        if self.comm_node is not None:
            print(f"    comm nodes: {self.comm_node.val}, idx={self.comm_node.__sort_idx__}")

    def show(self):
        print("comm comp overlap head:")
        self._show()
